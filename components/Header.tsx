...
interface HeaderProps {
  onNavigate: (mode: 'home' | 'resume' | 'cover_letter' | 'resignation' | 'ats_scorer') => void;
}
    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-100 font-['Inter Tight']">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          {/* Logo */}
          <button onClick={() => onNavigate('home')} className="flex items-center group cursor-pointer focus:outline-none">
            <img 
          ...
          <div className="hidden lg:flex items-center gap-3">
            <a 
              ...
              className="px-5 py-2.5 text-sm font-semibold text-[#191929]/70 hover:text-[#191929] transition-colors tracking-tight"
            >
              Log In
            </a>
            <a 
          ...
          <div className="lg:hidden">
            <button
              ...
              className="p-2 rounded-md text-[#475569] hover:text-[#191929] hover:bg-gray-100"
            >
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 py-4 px-4 space-y-4 animate-in slide-in-from-top-2">
          {navData.map((nav, index) => (
            <div key={index} className="space-y-2">
              <div className="font-bold text-[#191929] px-2 tracking-tight">{nav.label}</div>
              <div className="grid grid-cols-1 gap-1 pl-4">
                {nav.items.map((item, idx) => (
                  <button 
                    key={idx} 
                    onClick={() => handleItemClick(item.label, nav)}
                    className="py-2 text-sm text-left text-[#191929]/60 hover:text-[#1918f0] tracking-tight"
                  >
                    {item.label}
                  </button>
                ))}
          ...
          <div className="pt-4 flex flex-col gap-3">
            <a href="https://app.zysculpt.com" className="w-full py-3 text-center font-semibold text-[#191929]/70 border border-gray-200 rounded-xl tracking-tight">
              Log In
            </a>

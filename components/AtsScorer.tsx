...
const AtsScorer: React.FC = () => {
  ...
  // Visibility states for collapsing/expanding sections
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-24 h-24">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-xl font-black text-[#191929]">{score}%</span>
        </div>
      </div>
      <span className="text-[10px] font-bold uppercase tracking-widest text-[#191929]/60">{label}</span>
    </div>
  );
  ...
  const ChoiceOption = ({ label, active, onClick }: any) => (
    ...
    >
      ...
      <span className={`flex-1 font-normal transition-colors ${active ? 'text-[#1918f0]' : 'text-[#191929]'}`}>{label}</span>
    </button>
  );
  ...
  const ChevronIcon = ({ expanded }: { expanded: boolean }) => (
    <svg 
      className={`w-8 h-8 text-[#191929]/40 transition-transform duration-300 ${expanded ? 'rotate-0' : '-rotate-90'}`} 
  ...
  return (
    <div className="bg-white font-['Inter Tight'] min-h-screen">
      <section className="bg-gray-50 border-b border-gray-100 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <h1 className="text-4xl lg:text-6xl font-semibold text-[#191929] leading-[1.15] tracking-[-0.04em] mb-6">
              ATS Resume Scorer
            </h1>
            <p className="text-lg text-[#191929]/60 max-w-xl">
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="space-y-2">
                  <button 
                    ...
                    className="flex items-center gap-3 w-full text-left transition-colors hover:bg-gray-50/50 p-1 rounded-lg"
                  >
                    <ChevronIcon expanded={isJdOpen} />
                    <h2 className="text-xl font-semibold text-[#191929]">How would you like to provide the job description?</h2>
                  </button>
                </div>
                ...
                {isJdOpen && (
                  <div className="space-y-8 animate-in fade-in duration-300">
                    ...
                    {jdInputType && (
                      <div className="animate-in fade-in slide-in-from-top-4 duration-300 p-6 bg-gray-50 border border-gray-200 rounded-[32px]">
                        {jdInputType === 'text' && (
                          <textarea
                            value={jobDescription}
                            onChange={(e) => setJobDescription(e.target.value)}
                            placeholder="Paste job details here..."
                            className="w-full h-64 bg-white border border-gray-200 rounded-2xl p-6 text-sm focus:outline-none focus:border-[#1918f0] transition-all"
                          />
                        )}
                        {jdInputType === 'file' && (
                          <div 
                            onClick={() => jdFileInputRef.current?.click()}
                            className="border-2 border-dashed border-gray-300 bg-white rounded-2xl p-16 text-center cursor-pointer hover:border-[#1918f0]/50 hover:bg-[#1918f0]/5 transition-all"
                          >
                            ...
                            <h4 className="font-bold text-[#191929] mb-2">{jdFiles.length > 0 ? jdFiles[0].name : "Select job description file"}</h4>
                            <p className="text-xs text-[#191929]/40 uppercase tracking-widest font-bold">PDF, DOCX, or Image</p>
                          </div>
                        )}
                      </div>
                    )}
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="space-y-2">
                  <button 
                    ...
                    className="flex items-center gap-3 w-full text-left transition-colors hover:bg-gray-50/50 p-1 rounded-lg"
                  >
                    <ChevronIcon expanded={isResumeOpen} />
                    <h2 className="text-xl font-semibold text-[#191929]">How would you like to provide your resume?</h2>
                  </button>
                </div>
                      ...
                      <div className="animate-in fade-in slide-in-from-top-4 duration-300 p-6 bg-gray-50 border border-gray-200 rounded-[32px]">
                        ...
                        {resumeInputType === 'file' && (
                          ...
                          >
                            ...
                            <h4 className="font-bold text-[#191929] mb-2">{resumeFiles.length > 0 ? resumeFiles[0].name : "Select resume file"}</h4>
                            <p className="text-xs text-[#191929]/40 uppercase tracking-widest font-bold">PDF or DOCX</p>
                          </div>
                        )}
                      </div>
                    )}
            ...
            {step === 3 && (
              <div className="space-y-12 py-10 text-center animate-in zoom-in duration-500">
                ...
                <div className="space-y-4">
                  <h2 className="text-3xl font-bold text-[#191929]">Ready for Analysis</h2>
                  <p className="text-[#191929]/60 max-sm mx-auto">We've gathered all your information. Click below to start the scan.</p>
                </div>
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-[10px] font-bold text-[#191929]/30 uppercase tracking-widest">Takes about 10-15 seconds</p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="flex items-center justify-between gap-6 pb-8 border-b border-gray-50">
                  <div className="space-y-1">
                    <h3 className="text-3xl font-bold text-[#191929]">Analysis Result</h3>
                    <p className="text-sm font-medium text-[#191929]/60">{results.summary}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-5xl font-black text-[#1918f0]">{results.overallScore}</div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-[#191929]/40">Overall Match</div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4">
                    <div className="space-y-3">
                      <h4 className="text-xs font-black uppercase tracking-widest text-[#191929]/60">Formatting Feedback</h4>
                      <ul className="space-y-2">
                        {results.findings.formattingIssues.map((issue: string, i: number) => (
                          <li key={i} className="flex gap-2 text-sm text-[#191929]/80 font-medium">
                ...
                <div className="pt-8 border-t border-gray-50 flex justify-center">
                  <button onClick={() => { setStep(1); setResults(null); setJdInputType(null); setResumeInputType(null); setIsJdOpen(true); setIsResumeOpen(true); }} className="text-sm font-bold text-[#1918f0] hover:underline">Start New Analysis</button>
                </div>
              </div>
            )}
          </div>
          ...
          {step < 4 && (
            <div className="px-10 py-6 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
              <button 
                ...
                disabled={step === 1}
                className={`text-sm font-bold transition-all ${step === 1 ? 'text-gray-300 pointer-events-none' : 'text-[#191929] hover:text-[#1918f0]'}`}
              >
                Back
              </button>
              {step < 3 && (
      ...
      <section className="bg-[#191929] py-20 px-4">
        <div className="max-w-4xl mx-auto text-center space-y-8">
           ...
           <button 
             ...
             className="bg-white text-[#191929] px-12 py-4 rounded-full font-bold shadow-2xl transition-all hover:bg-gray-100 hover:scale-105"
           >
             Open AI Builder
           </button>
        </div>

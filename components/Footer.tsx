...
interface FooterProps {
  onNavigate: (mode: 'home' | 'resume' | 'cover_letter' | 'resignation' | 'interview_prep' | 'jobs' | 'ats_scorer') => void;
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-12 mb-16">
          {footerLinks.map((column, idx) => (
            <div key={idx}>
              <h4 className="text-sm font-bold text-[#191929] uppercase tracking-widest mb-6">
                {column.title}
              </h4>
                ...
                {column.links.map((link, linkIdx) => (
                  <li key={linkIdx}>
                    <button 
                      ...
                      className="text-[#191929]/60 hover:text-[#1918f0] text-sm transition-colors tracking-tight text-left"
                    >
                      {link.label}
                    </button>
                  </li>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <p className="text-[#191929]/40 text-sm tracking-tight">
            © {currentYear} Zysculpt All Rights Reserved.
          </p>

          <div className="flex gap-6 text-[#191929]/40">
            <a href="#" className="hover:text-[#191929] transition-colors">
